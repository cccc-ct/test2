...
#include "sys.h"
#include "usart.h"
	volatile u8 usart1_rx;
	volatile u8 usart1_rx_flag = 0;
...
{	
	...
	if(USART_GetITStatus(USART1,USART_IT_RXNE)!=RESET)
	{
		...
		usart1_rx=USART_ReceiveData(USART1);
		usart1_rx_flag=1;
		//USART_SendData(USART1,res);
		USART_SendData(USART1,usart1_rx);
	}
}
#endif	
...
//		USART2_printf("%c",Res2); //把收到的数据以 a符号变量 发送回电脑		
...
//				else{
//					USART2_RX_BUF[USART2_RX_STA&0X3FFF]=Res2 ; //将收到的数据放入数组
